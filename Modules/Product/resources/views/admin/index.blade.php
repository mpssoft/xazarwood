...
@section('content')
    ...
    <div class="bg-gray-50 dark:bg-slate-900 text-gray-900 dark:text-slate-100 transition-colors duration-300">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-6">
            <header
                class="bg-white dark:bg-slate-800 shadow-lg border border-gray-200 dark:border-slate-700 rounded-xl">
                <div class="px-6 py-4">
                    <div class="flex justify-between items-center">
                        <!-- Brand -->
                        <div class="flex items-center space-x-4 space-x-reverse">
                            <div
                                class="w-12 h-12 bg-blue-600 dark:bg-slate-600 rounded-xl flex items-center justify-center">
                        ...
                        <div class="flex items-center space-x-4 space-x-reverse">
                            ...
                            <a href="{{route('admin.products.create')}}" id="add-product-btn"
                               class="bg-blue-600 dark:bg-slate-600 hover:bg-blue-700 dark:hover:bg-slate-500 text-white px-4 py-2 rounded-lg font-medium transition-colors">
        ...
        <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
            ...
            <!-- Filters -->
            <div
                class="bg-white dark:bg-slate-800 rounded-xl p-6 shadow-sm border border-gray-200 dark:border-slate-700 mb-4">
                <div class="flex flex-col md:flex-row gap-4">
                    <div class="flex-1">
                        <label for="search" class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">جستجو</label>
                        <div class="relative">
                            <input type="text" id="search" placeholder="نام محصول را جستجو کنید..."
                                   class="w-full pl-10 pr-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100">
                    ...
                    <div>
                        <label for="status-filter"
                               class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">دسته</label>
                        <select id="status-filter" onchange="getProducts(this.value)"
                                class="px-4 py-2 border border-gray-300 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-slate-700 text-gray-900 dark:text-slate-100">
                            <option value="all">همه</option>
                            @foreach(\Modules\Blog\Models\Category::all() as $category)
                                <option
                                    {{$category->id==$cat ? 'selected' :''}}  value="{{$category->id}}">{{$category->name}}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        <label for="sort-filter"
                               class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">مرتب‌سازی</label>
                    ...
                    <div>
                        ...
                        <div class="flex  flex-1   p-2 gap-3   text-[10px] items-center justify-center">
                            ...
                            <a href="{{route('admin.products.index',['layout'=>'card', 'category'=>$cat ] )}}">
            ...
            @if($layout =='list')
                @include('product::admin.list-view-index',$products)
            @else
                @include('product::admin.card-view-index',$products)
            @endif
            <!-- Products Grid -->
            <div id="products-grid" class="md:hidden grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                @foreach($products as $product)
                    <div
                        class="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-gray-200 dark:border-slate-700 overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
                        <!-- Product Image -->
                        <div class="relative h-48 bg-gray-100 dark:bg-slate-700"
                             style="background:url('{{asset($product->main_image)}}');background-size:cover">

                            <div class="absolute top-3 right-3">
                                @if($product->stock > 0)
                                    <span
                                        class="bg-green-100 dark:bg-slate-700 text-green-800 dark:text-slate-300 px-2 py-1 rounded-full text-xs font-medium animate-pulse">
                            موجود
                        </span>
                                @else
                                    <span
                                        class="bg-red-100 dark:bg-red-700 text-red-800 dark:text-red-300 px-2 py-1 rounded-full text-xs font-medium animate-pulse">
                            نا موجود
                        </span>
                                @endif
                            </div>
                            <div class="absolute top-3 left-3 flex space-x-2 space-x-reverse">
                                <a href="{{route('admin.products.edit',$product->id)}}"
                                   class="p-2 bg-white dark:bg-slate-800 rounded-full shadow-md hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors">
                                    <i class="fas fa-edit text-blue-600 dark:text-slate-300 text-sm"></i>
                                </a>
                                <form action="{{ route('admin.products.destroy',$product->id) }}"
                                      onsubmit="event.preventDefault();confirmDelete(event);" method="post"
                                      id="{{'delete-'.$product->id}}">@csrf @method('delete')
                        ...
                        <div class="p-6">
                            ...
                            <div class="border-t border-gray-200 dark:border-slate-700 pt-4">
                                <div class="flex justify-between items-center mb-3">
                                    <span
                                        class="text-sm font-medium text-gray-700 dark:text-slate-300">گالری تصاویر</span>
                                    <button
                                        class="text-blue-600 dark:text-slate-300 hover:text-blue-700 dark:hover:text-slate-200 text-sm font-medium">
                                ...
                                <div class="grid grid-cols-3 gap-2">
                                    ...
                                    @foreach($product->images as $image)
                                        <div
                                            class="aspect-square bg-gray-100 dark:bg-slate-700 rounded-lg flex items-center justify-center relative group">
