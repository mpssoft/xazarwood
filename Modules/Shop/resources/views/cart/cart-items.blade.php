...
@if( count($cart))
    ...
    @foreach($cart as $item)
        ...
        <div class="cart-item flex items-center gap-3 p-3 border border-wood-300 dark:border-wood-600 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200">
            <div class="w-12 h-12 bg-gradient-to-br from-amber-400 to-orange-600 rounded-full flex items-center justify-center flex-shrink-0">
                <i class="fas fa-shopping-bag text-white text-lg"></i>
            </div>
            <div class="flex-1 min-w-0">
                <h4 class="font-medium text-sm truncate text-wood-800 dark:text-wood-100">{{$item['model']['title']}}</h4>
                @if(!is_null($item['discount']))
                    ...
                    <div class="flex items-center gap-2 mt-1">
                        ...
                        <span class="text-xs bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400 px-1.5 py-0.5 rounded-full">
    ...
    <!-- Checkout Button -->
    <a href="/cart" id="checkoutButton" class="w-full btn btn-primary dark:w-full mt-2 dark:mt-2 bg-gradient-to-r from-amber-600 to-orange-600 text-white py-3 rounded-xl font-bold text-sm hover:shadow-lg transition-all duration-200 hover:scale-105 block text-center">
