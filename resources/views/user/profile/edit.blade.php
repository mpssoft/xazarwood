...
@section('content')
<div class="bg-gradient-to-br from-indigo-50 to-sky-50 dark:from-wood-900 dark:to-wood-900 min-h-screen py-8 px-4">
<div class="max-w-4xl mx-auto space-y-8">
    ...
    <div class="bg-white/90 dark:bg-wood-800/90 backdrop-blur rounded-2xl shadow-xl border border-indigo-100 dark:border-wood-700 overflow-hidden">
        <div class="p-8">
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl md:text-3xl font-extrabold text-wood-900 dark:text-white">ویرایش پروفایل</h1>
                    <p class="text-wood-600 dark:text-wood-300">اطلاعات شخصی خود را به‌روزرسانی کنید</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <!-- Profile Form -->
    <div class="bg-white dark:bg-wood-800 rounded-2xl shadow-xl border border-indigo-100 dark:border-wood-700 overflow-hidden">
        <form id="profileForm" class="p-8 space-y-8" method="post" action="{{route('user.profile.update')}}" enctype="multipart/form-data">
            ...
            <div class="text-center">
                    ...
                    <div class="profile-photo-upload relative w-32 h-32 mx-auto mb-4 cursor-pointer group" onclick="document.getElementById('photoInput').click()">
                        @if(auth()->user()->image)
                            <img id="profileImage" src="{{\Illuminate\Support\Facades\Storage::disk('users')->url( 'thumbs/'.auth()->user()->image)}}" width="100"
                                 alt="تصویر پروفایل"
                                 class="w-32 h-32 rounded-full object-cover border-4 border-white dark:border-wood-700 shadow-lg">
                ...
                <p class="text-sm text-wood-500 dark:text-wood-400">روی تصویر کلیک کنید تا عکس جدید انتخاب کنید</p>
                <p class="text-xs text-wood-400 dark:text-wood-500 mt-1">فرمت‌های مجاز: JPG, PNG, GIF (حداکثر 5MB)</p>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div class="form-field space-y-2">
                    <label for="name" class="block text-sm font-medium text-wood-700 dark:text-wood-300">
                        <i class="fas fa-user text-indigo-500 ml-2"></i>
                        نام
                    </label>
                    <input type="text" id="name" name="name" value="{{auth()->user()->name}}" required
                           class="w-full px-4 py-3 rounded-xl border border-wood-300 dark:border-wood-600 bg-white dark:bg-wood-700 text-wood-900 dark:text-wood-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition">
                ...
                <div class="form-field space-y-2">
                    <label for="family" class="block text-sm font-medium text-wood-700 dark:text-wood-300">
                        <i class="fas fa-user text-indigo-500 ml-2"></i>
                        نام خانوادگی
                    </label>
                ...
                <div class="form-field space-y-2">
                    <label for="email" class="block text-sm font-medium text-wood-700 dark:text-wood-300">
...
<!-- Email Field -->
                ...
                <div class="form-field space-y-2">
                    <label for="mobile" class="block text-sm font-medium text-wood-700 dark:text-wood-300">
                        <i class="fas fa-mobile-alt text-wood-400 ml-2"></i>
                        شماره موبایل
                    </label>
                    <input type="tel" id="mobile" name="mobile" value="{{auth()->user()->mobile}}" disabled
                           class="w-full px-4 py-3 rounded-xl border border-wood-300 dark:border-wood-600 bg-wood-100 dark:bg-wood-600 text-wood-500 dark:text-wood-400 cursor-not-allowed">
                    <div class="text-xs text-wood-500 dark:text-wood-400">
...
{{--
                <!-- Additional Info -->
                <div class="form-field space-y-2">
                    <label for="bio" class="block text-sm font-medium text-wood-700 dark:text-wood-300">
                    ...
                    <textarea id="bio" name="bio" rows="3" placeholder="چند کلمه درباره خودتان بنویسید..."
                              class="w-full px-4 py-3 rounded-xl border border-wood-300 dark:border-wood-600 bg-white dark:bg-wood-700 text-wood-900 dark:text-wood-100 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition resize-none">توسعه‌دهنده نرم‌افزار با علاقه به یادگیری تکنولوژی‌های جدید</textarea>
                </div>--}}
            </div>

            <!-- Account Settings -->
            {{--<div class="border-t border-wood-200 dark:border-wood-700 pt-8">
                <h3 class="text-lg font-semibold text-wood-900 dark:text-white mb-6">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    <!-- Notifications -->
                    <div class="space-y-4">
                        <h4 class="font-medium text-wood-700 dark:text-wood-300">اعلان‌ها</h4>
                        <div class="space-y-3">
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" checked class="w-5 h-5 text-indigo-600 rounded focus:ring-indigo-500">
                                <span class="text-sm text-wood-700 dark:text-wood-300">اعلان‌های ایمیل</span>
                            </label>
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" checked class="w-5 h-5 text-indigo-600 rounded focus:ring-indigo-500">
                                <span class="text-sm text-wood-700 dark:text-wood-300">اعلان‌های پیامک</span>
                            </label>
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" class="w-5 h-5 text-indigo-600 rounded focus:ring-indigo-500">
                                <span class="text-sm text-wood-700 dark:text-wood-300">اعلان‌های بازاریابی</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <!-- Privacy -->
                    <div class="space-y-4">
                        <h4 class="font-medium text-wood-700 dark:text-wood-300">حریم خصوصی</h4>
                        <div class="space-y-3">
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" checked class="w-5 h-5 text-indigo-600 rounded focus:ring-indigo-500">
                                <span class="text-sm text-wood-700 dark:text-wood-300">نمایش پروفایل عمومی</span>
                            </label>
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" class="w-5 h-5 text-indigo-600 rounded focus:ring-indigo-500">
                                <span class="text-sm text-wood-700 dark:text-wood-300">نمایش فعالیت‌ها</span>
                            </label>
                        </div>
                    </div>
                </div>
